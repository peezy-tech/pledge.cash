[Back to Components](https://www.convex.dev/components) ![Migrations hero image](https://www.convex.dev/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fmigrations.1c52c92c.png&w=1536&q=75)
...
You can then run it programmatically or from the CLI. See [below](https://www.convex.dev/components/#running-migrations-one-at-a-time).
...
See [below](https://www.convex.dev/components/#shorthand-running-syntax) for a way to just pass `setDefaultValue`.
...
- If you pass `true` for `dryRun` then it will run one batch and then throw, so no changes are committed, and you can see what it would have done. See [below](https://www.convex.dev/components/#test-a-migration-with-dryrun) This is good for validating it does what you expect.
...
See [this article](https://stack.convex.dev/migrating-data-with-mutations) for more information on usage and advanced patterns.
