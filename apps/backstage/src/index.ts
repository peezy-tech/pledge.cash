...
const app = new Elysia()
  ...
  .get("/auth/twitter", async ({ oauth2, redirect }: { oauth2: any, redirect: any }) => {
    const url = await oauth2.createURL("Twitter", ["users.read", "tweet.read"]);
    return redirect(url.href);
  })
  .get("/auth/twitter/callback", async ({ oauth2, query }: { oauth2: any, query: { code?: string, state?: string, error?: string, error_description?: string } }) => {
    if (query.error) {
      console.error(
    ...
    if (!query.code) {
      console.error("Twitter OAuth error: No code provided in callback query.");
      return {
        error: "Twitter OAuth failed",
