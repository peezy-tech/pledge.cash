...
contract Treasury is Ownable, ReentrancyGuard, Test {
    using SafeERC20 for IERC20;

    // Immutable variables for pool configuration
    ...
    event PoolSeeded(uint256 amount0, uint256 amount1);
    ...
    constructor(
        ...
        factory = IPoolFactory(router.defaultFactory());

        // Get existing pool or create new one
        address poolAddress = factory.getPool(_token0, _token1, _isStable);
        if (poolAddress == address(0)) {
            poolAddress = factory.createPool(_token0, _token1, _isStable);
        }
        pool = poolAddress;
    }

    /**
     * @notice Seeds the pool with initial liquidity
     * @param amount0 Amount of token0 to seed
     * @param amount1 Amount of token1 to seed
     */
    function seedPool(
        uint256 amount0,
        uint256 amount1
        ...
        // Check allowances
        if (IERC20(token0).allowance(msg.sender, address(this)) < amount0)
            revert InsufficientAllowance();
        if (IERC20(token1).allowance(msg.sender, address(this)) < amount1)
            revert InsufficientAllowance();
        ...
        (uint256 actualAmt0, uint256 actualAmt1, uint256 liqTokens) = router
            .quoteAddLiquidity(
                token0,
                token1,
                false,
        ...
        console.log("liq expected", liqTokens);
        console.log("amts expected", actualAmt0);
        console.log("amts expected", actualAmt1);
        ...
        console.log("pool supply", IPool(pool).totalSupply());

        emit PoolSeeded(amount0, amount1);
    }
    ...
    {
        (amount0, amount1) = IPool(pool).claimFees();
        // TODO: send to owner?
        emit FeesCollected(amount0, amount1);
    }

    /**
     * @notice View function to check claimable fees
     */
    function getClaimableFees()
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        amount0 = IPool(pool).claimable0(address(this));
        amount1 = IPool(pool).claimable1(address(this));
    }
