...
contract LockedVaultTest is Test { // Removed EIP712 inheritance
    LockedVault vault;
    ...
    uint256 directorPrivateKey = 0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80;
    uint256 auctioneerPrivateKey = 0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d;
    ...
    function _signWithdrawPermit(
        ...
        bytes32 digest = vault.getWithdrawPermitDigest(owner, token, amount, nonce, validAfter);
    ...
    function _signAuctioneerSettlement(
        ...
        bytes32 digest = vault.getAuctionSettlementDigest(auctionId, root, price);
    ...
    function test_Constructor() public view { // Changed to view
    ...
    function test_Deposit() public {
        ...
        assertEq(stableToken.balanceOf(address(vault)), depositAmount, "Vault stablecoin balance incorrect");
        assertEq(vault.balance(stableToken, alice), depositAmount, "Alice's vault balance incorrect");
    }
    ...
    function test_CreateAuction() public {
        ...
        uint256 auctionId = vault.createAuction(sellToken, auctionAmount, stableToken);
        vm.stopPrank();

        assertEq(auctionId, 1, "First auction ID should be 1");
        assertEq(sellToken.balanceOf(address(vault)), auctionAmount, "Vault sellToken balance incorrect after auction creation");
        
        // Using commas for unused destructured variables r and p
        (IERC20 sTok, uint256 amt, IERC20 stbl, address auc, /*bytes32 r*/, /*uint256 p*/, bool settld) = vault.auctions(auctionId);
    ...
    function test_SettleAuction() public {
        ...
        bytes memory auctioneerSig = _signAuctioneerSettlement(auctionId, testRoot, clearingPrice);
        ...
        (,,,, bytes32 fetchedRoot, uint256 fetchedPrice, bool isSettled) = vault.auctions(auctionId);
        ...
        assertTrue(vault.lastAuctionTime() >= timeBeforeSettle + 1 days, "lastAuctionTime not updated correctly");
    }
    ...
    function test_Revert_Settle_NotDirector() public {
        ...
        vm.startPrank(auctioneer); sellToken.approve(address(vault), 1 ether);
        auctionId = vault.createAuction(sellToken, 1 ether, stableToken); vm.stopPrank();
    ...
    function test_Revert_Settle_BadAuctioneerSignature() public {
        ...
        bytes32 testRoot = keccak256(abi.encodePacked("merkleRoot"));
        uint256 clearingPrice = 2 ether;
        bytes memory badSig = _signAuctioneerSettlement(auctionId + 1, testRoot, clearingPrice);
    ...
    function test_Claim() public {
        ...
        bytes32 leaf = keccak256(abi.encode(alice, aliceFillAmount, alicePayAmount, claimNonce));
        ...
        bytes memory auctioneerSig = _signAuctioneerSettlement(auctionId, merkleRoot, clearingPrice);
        ...
        vm.startPrank(alice);
        vault.claim(auctionId, aliceFillAmount, alicePayAmount, claimNonce, proof);
        vm.stopPrank();

        assertTrue(vault.claimed(auctionId, alice), "Alice's claim not marked");
        assertEq(vault.balance(stableToken, alice), aliceStableBalanceBefore - alicePayAmount, "Alice stable balance after claim incorrect");
        assertEq(vault.balance(sellToken, alice), aliceSellBalanceBefore + aliceFillAmount, "Alice sell token balance after claim incorrect");
    }
    ...
    function test_Revert_Claim_BadProof() public {
        ...
        uint256 auctionId = vault.createAuction(sellToken, 50 ether, stableToken);
        vm.stopPrank();

        bytes32 actualLeaf = keccak256(abi.encode(alice, 10 ether, 20 ether, 123));
        bytes32 merkleRoot = actualLeaf; 
        bytes memory auctioneerSig = _signAuctioneerSettlement(auctionId, merkleRoot, 2 ether);
        ...
        badProof[0] = keccak256(abi.encodePacked("some other hash for bad proof"));
    ...
    function test_Withdraw() public {
        ...
        uint256 auctionId = vault.createAuction(sellToken, 1 ether, stableToken);
        vm.stopPrank();
        
        bytes32 dummyRoot = keccak256(abi.encodePacked("root"));
        bytes memory sigAuctioneer = _signAuctioneerSettlement(auctionId, dummyRoot, 1 ether);
        ...
        bytes memory directorSig = _signWithdrawPermit(alice, stableToken, withdrawAmount, nonce, validAfter);
        ...
        assertEq(stableToken.balanceOf(alice), aliceTokenBalanceBefore + withdrawAmount, "Alice's external balance incorrect");
        assertEq(vault.balance(stableToken, alice), vaultAliceBalanceBefore - withdrawAmount, "Alice's vault balance incorrect");
    }
    ...
    function test_Revert_Withdraw_StaleSignature() public {
        ...
        bytes memory directorSig = _signWithdrawPermit(alice, stableToken, withdrawAmount, nonce, staleValidAfter);

        vm.startPrank(alice);
        vm.expectRevert("sig stale");
        vault.withdraw(stableToken, withdrawAmount, staleValidAfter, directorSig);
        vm.stopPrank();
    }
    ...
    function test_Revert_Withdraw_BadDirectorSignature() public {
        ...
        vm.startPrank(auctioneer); sellToken.approve(address(vault), 1 ether);
        uint256 auctionId = vault.createAuction(sellToken, 1 ether, stableToken); vm.stopPrank();
        bytes32 r = keccak256(abi.encodePacked("r")); uint256 p = 1 ether;
        bytes memory sA = _signAuctioneerSettlement(auctionId, r, p);
        vm.startPrank(director); vault.settle(auctionId, r, p, sA); vm.stopPrank();
        ...
        bytes32 digest = vault.getWithdrawPermitDigest(alice, stableToken, withdrawAmount, nonce, validAfter);
        (uint8 v, bytes32 rSig, bytes32 sSig) = vm.sign(auctioneerPrivateKey, digest); // Signed by wrong key (auctioneer)
    ...
    function test_PreviewWithdraw() public {
        ...
        assertEq(vault.previewWithdraw(bob, stableToken), depositAmount, "Preview amount incorrect");
    }
}
