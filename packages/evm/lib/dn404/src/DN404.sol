...
abstract contract DN404 {
    ...
    function _mintNft(address to, uint256 amount) internal virtual {
        if (to == address(0)) _rv(uint32(TransferToZeroAddress.selector));
        ...
        _DNMintTemps memory t;
        t.toEnd = toAddressData.ownedLength + amount;
        if (t.toEnd > toAddressData.balance / _unit()) revert InsufficientBalance();
