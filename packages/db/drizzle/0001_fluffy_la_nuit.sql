CREATE TABLE `invoice_hooks` (
	...
	`type` text NOT NULL,
	`url` text NOT NULL,
	`createdAt` integer DEFAULT 1751705512369 NOT NULL,
...
--> statement-breakpoint
INSERT INTO `__new_hyperliquid_invoices`("id", "creatorId", "payerAddress", "token", "amount", "description", "status", "txHash", "createdAt", "paidAt", "expiresAt") SELECT "id", "creatorId", "payerAddress", "token", "amount", "description", "status", "txHash", "createdAt", "paidAt", "expiresAt" FROM `hyperliquid_invoices`;--> statement-breakpoint
...
CREATE TABLE `__new_tx_hashes` (
	...
	`createdAt` integer DEFAULT 1751705512368 NOT NULL,
	`metadata` text
);
--> statement-breakpoint
