...
interface ILockedLiquidityFactoryVNextBoardroom {
...
interface ILockedLiquidityFactoryVNextPolicyRegistry {
    function isModulePolicy(address policy) external view returns (bool);
}

interface ILockedLiquidityFactoryVNextBoardroomFactory {
...
interface ILockedLiquidityFactoryVNextShareToken {
...
contract LockedLiquidityFactoryVNext is IBoardroomObligationPolicy, ReentrancyGuard {
    ...
    function closeLockedLiquidity() external nonReentrant {
        ...
        BoardroomVNextCallbackLib.closeProtocolLiquidityFromFactory(boardroom, position.locker);
    ...
    function finalizeWindDownClosure() external nonReentrant {
        ...
        if (
            position.status != PositionStatus.Active || migrationReservationOf[boardroom].curve != address(0)
                || !ILockedLiquidityFactoryVNextBoardroom(boardroom).lockedLiquidityExitAllowed()
        ...
        nonReentrant
    {
        ...
        BoardroomVNextCallbackLib.releaseProtocolLiquidityReservation(boardroom, msg.sender, reservation.pairKey, salt);
        ...
        returns (bool)
    {
        ...
        if (target == address(this)) {
            if (
                selector == LockedLiquidityFactoryVNext.createLockedLiquidity.selector
                    && data.length == CREATE_DATA_LENGTH
            ) {
                return _canCreate(boardroom, abi.decode(data[4:], (CreateParams)));
            }
            if (selector == LockedLiquidityFactoryVNext.addLockedLiquidity.selector && data.length == ADD_DATA_LENGTH) {
                return _canAdd(boardroom, abi.decode(data[4:], (AddParams)));
            }
            if (
                selector == LockedLiquidityFactoryVNext.removeLockedLiquidity.selector
            ...
            if (selector == LockedLiquidityFactoryVNext.closeLockedLiquidity.selector && data.length == 4) {
    ...
    {
        if (
            target != address(this) || _selector(data) != LockedLiquidityFactoryVNext.createLockedLiquidity.selector
        ...
        returns (address locker, address pool, uint256 amountA, uint256 amountB, uint256 liquidity)
    {
        ...
        BoardroomVNextCallbackLib.activateProtocolLiquidity(
            boardroom, locker, pool, quoteAsset, curve, pairKey, params.salt
        );
    ...
    function _requireAuthorizedMigrationFactory(
        ...
        _requireCanonicalBoardroom(boardroom);
        address registry = ILockedLiquidityFactoryVNextBoardroom(boardroom).policyRegistry();
        if (!ILockedLiquidityFactoryVNextPolicyRegistry(registry).isModulePolicy(msg.sender)) {
            revert InvalidMigrationReservation(boardroom, curve);
        }
    ...
    function _requireIssuedDistribution(address boardroom, address curve) internal view {
        _requireCanonicalBoardroom(boardroom);
        if (!ILockedLiquidityFactoryVNextBoardroom(boardroom).isIssuedDistribution(curve)) {
            revert InvalidMigrationReservation(boardroom, curve);
        }
    }
    ...
    function _isCanonicalBoardroom(address boardroom) internal view returns (bool) {
        return boardroom.code.length != 0
            && ILockedLiquidityFactoryVNextBoardroomFactory(boardroomFactory).isBoardroom(boardroom);
    }
    ...
    function _validPair(address boardroom, address tokenA, address tokenB) internal view returns (bool) {
        ...
        return !ILockedLiquidityFactoryVNextBoardroomFactory(boardroomFactory).isShareToken(quoteAsset);
    }
    ...
    function _boardroomShareToken(address boardroom) internal view returns (address shareToken) {
        shareToken = ILockedLiquidityFactoryVNextBoardroom(boardroom).shareToken();
        if (shareToken == address(0) || ILockedLiquidityFactoryVNextShareToken(shareToken).boardroom() != boardroom) {
