...
interface IBondMarketFactoryVNextBoardroom {
    function shareToken() external view returns (address);
}

interface IBondMarketFactoryVNextBoardroomFactory {
    function isBoardroom(address boardroom) external view returns (bool);
}

interface IBondMarketFactoryVNextAmmFactory {
    function isPool(address pool) external view returns (bool);

    function boardroomFactory() external view returns (address);
}

interface IBondMarketFactoryVNextPool {
    function tokens() external view returns (address token0, address token1);
    function totalSupply() external view returns (uint256);
}

contract BondMarketFactoryVNext is IBoardroomObligationPolicy {
    ...
    constructor(address ammFactory_, address boardroomFactory_) {
        ...
        address ammBoardroomFactory = IBondMarketFactoryVNextAmmFactory(ammFactory_).boardroomFactory();
    ...
    function createBondMarket(BondMarket.CreateParams calldata params) external returns (address market) {
        ...
        if (!IBondMarketFactoryVNextBoardroomFactory(boardroomFactory).isBoardroom(boardroom)) {
            revert InvalidBoardroom(boardroom);
        }

        address shareToken = IBondMarketFactoryVNextBoardroom(boardroom).shareToken();
        _validateQuoteToken(params.quoteToken, shareToken, params.kind);
        BoardroomVNextCallbackLib.reserveRedeemableAsset(boardroom, params.quoteToken);
        ...
        returns (bool)
    {
        ...
        if (target == address(this)) {
            if (selector != BondMarketFactoryVNext.createBondMarket.selector || data.length != CREATE_DATA_LENGTH) {
                return false;
            }
            if (!IBondMarketFactoryVNextBoardroomFactory(boardroomFactory).isBoardroom(boardroom)) return false;
    ...
    {
        if (
            target != address(this) || _selector(data) != BondMarketFactoryVNext.createBondMarket.selector
                || result.length != 32
        ) return obligation;
    ...
    function _canCreate(address boardroom, BondMarket.CreateParams memory params) internal view returns (bool) {
        address shareToken;
        try IBondMarketFactoryVNextBoardroom(boardroom).shareToken() returns (address token) {
            shareToken = token;
        } catch {
            return false;
        }
        ...
        returns (bool)
    {
        ...
        bool pool = IBondMarketFactoryVNextAmmFactory(ammFactory).isPool(quoteToken);
        if (kind == BondMarket.MarketKind.Reserve) return !pool;
        if (!pool) return false;

        try IBondMarketFactoryVNextPool(quoteToken).tokens() returns (address token0, address token1) {
            if (token0 != shareToken && token1 != shareToken) return false;
        } catch {
            return false;
        }

        try IBondMarketFactoryVNextPool(quoteToken).totalSupply() returns (uint256 supply) {
            return supply > 1_000;
        } catch {
            return false;
        }
