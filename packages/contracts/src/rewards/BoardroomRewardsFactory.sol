...
contract BoardroomRewardsFactory is IBoardroomObligationPolicy {
    ...
    function createRewards(uint64 cooldown, bytes32 salt) external returns (address rewards) {
        ...
        address shareToken = IBoardroomRewardsFactoryBoardroom(boardroom).shareToken();
    ...
    function fundReward(address rewards, address asset, uint256 amount, uint256 duration) external {
        ...
        if (asset == address(0) || asset == IBoardroomRewardsFactoryBoardroom(boardroom).shareToken()) {
        ...
        returns (bool)
    {
        ...
        if (target == address(this)) {
            if (selector == BoardroomRewardsFactory.createRewards.selector) {
                return data.length == CREATE_DATA_LENGTH && rewardsForBoardroom[boardroom] == address(0)
                    && _isActiveBoardroom(boardroom);
            }
            if (selector == BoardroomRewardsFactory.fundReward.selector && data.length == FUND_DATA_LENGTH) {
                ...
                return rewardsBoardroom[rewards] == boardroom && asset != address(0)
                    && asset != IBoardroomRewardsFactoryBoardroom(boardroom).shareToken() && amount != 0
        ...
        returns (Obligation memory obligation)
    {
        if (
            target != address(this) || data.length != CREATE_DATA_LENGTH || result.length != 32
                || _selector(data) != BoardroomRewardsFactory.createRewards.selector
        ) return obligation;
    ...
    function _requireBoardroom(address boardroom) internal view {
        if (!IBoardroomRewardsFactoryBoardroomFactory(boardroomFactory).isBoardroom(boardroom)) {
    ...
    function _isActiveBoardroom(address boardroom) internal view returns (bool) {
        if (!IBoardroomRewardsFactoryBoardroomFactory(boardroomFactory).isBoardroom(boardroom)) return false;
        try IBoardroomRewardsFactoryBoardroom(boardroom).status() returns (uint8 status) {
