...
interface IMigratingBondingCurveVNextBoardroom {
...
contract MigratingBondingCurveVNext is Initializable, ReentrancyGuard {
        ...
        returns (address createdLocker, address createdPool, uint256 amountA, uint256 amountB, uint256 liquidity)
    {
        ...
        BoardroomVNextCallbackLib.recordLockedLiquidityFromDistribution(boardroom, createdLocker, createdPool);
    ...
    function fallbackToUnwind() external nonReentrant {
        ...
        uint8 boardroomStatus = IMigratingBondingCurveVNextBoardroom(boardroom).status();
    ...
    function vetoQuoteForfeiture() external nonReentrant {
        ...
        IMigratingBondingCurveVNextBoardroom(boardroom).requireBondingCurveForfeitureVetoPower(msg.sender);
    ...
    function canMigrate() public view returns (bool) {
        if (
            migrationInProgress || curveStatus != CurvePhase.Graduated || block.timestamp > phaseEndsAt
                || IMigratingBondingCurveVNextBoardroom(boardroom).status() != BOARDROOM_STATUS_ACTIVE
        ) return false;
    ...
    function _requireActiveBoardroom() internal view {
        if (IMigratingBondingCurveVNextBoardroom(boardroom).status() != BOARDROOM_STATUS_ACTIVE) {
            revert BoardroomNotActive();
        }
    }

    function _requireBoardroomBeforeSnapshot() internal view {
        if (IMigratingBondingCurveVNextBoardroom(boardroom).status() > BOARDROOM_STATUS_WINDING_DOWN) {
    ...
    function _recoveryRecipient() internal view returns (address) {
        if (IMigratingBondingCurveVNextBoardroom(boardroom).status() >= BOARDROOM_STATUS_SNAPSHOTTING) {
            return IMigratingBondingCurveVNextBoardroom(boardroom).redemptionExcessRecipient();
        }
        return boardroom;
    }
