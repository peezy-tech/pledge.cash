...
import {MerkleAirdropVNext} from "./MerkleAirdropVNext.sol";
import {MigratingBondingCurveVNext} from "./MigratingBondingCurveVNext.sol";
import {LockedLiquidityFactoryVNext} from "../liquidity/LockedLiquidityFactoryVNext.sol";
...
interface IDistributionVNextBoundFactory {
    function boardroomFactory() external view returns (address);
}

interface IDistributionVNextBoardroomFactory {
    function isBoardroom(address boardroom) external view returns (bool);
}

contract DistributionFactoryVNext is IBoardroomObligationPolicy {
    ...
    constructor(address lockedLiquidityFactory_, address tokenGrantFactory_) {
        ...
        address expectedBoardroomFactory = IDistributionVNextBoundFactory(tokenGrantFactory_).boardroomFactory();
        address liquidityBoardroomFactory = IDistributionVNextBoundFactory(lockedLiquidityFactory_).boardroomFactory();
        ...
        migratingBondingCurveLogic = address(new MigratingBondingCurveVNext());
        merkleAirdropLogic = address(new MerkleAirdropVNext());
    }
    ...
    function createFixedPriceSale(FixedPriceSale.CreateParams calldata params) external returns (address sale) {
        ...
        BoardroomVNextCallbackLib.reserveRedeemableAsset(msg.sender, params.paymentToken);
    ...
    function createMigratingBondingCurve(MigratingBondingCurveVNext.CreateParams calldata params)
        external
        returns (address curve)
    {
        ...
        BoardroomVNextCallbackLib.reserveRedeemableAsset(msg.sender, params.quoteToken);
        ...
        BoardroomVNextCallbackLib.precommitBondingCurve(msg.sender, predictedCurve, params.quoteToken, shareAmount);
        ...
        MigratingBondingCurveVNext(curve).initialize(msg.sender, lockedLiquidityFactory, params);
        LockedLiquidityFactoryVNext(lockedLiquidityFactory)
    ...
    function createMerkleAirdrop(MerkleAirdropVNext.CreateParams calldata params) external returns (address airdrop) {
        _requireBoardroomShareToken(msg.sender, params.shareToken);
        ...
        MerkleAirdropVNext(airdrop).initialize(msg.sender, tokenGrantFactory, params);
    ...
    function _canCreateDistribution(address boardroom, bytes4 selector, bytes calldata data)
        ...
        returns (bool)
    {
        if (selector == DistributionFactoryVNext.createFixedPriceSale.selector) {
            return _canCreateFixedPriceSale(boardroom, data);
        }
        if (selector == DistributionFactoryVNext.createMigratingBondingCurve.selector) {
            return _canCreateMigratingBondingCurve(boardroom, data);
        }
        if (selector == DistributionFactoryVNext.createMerkleAirdrop.selector) {
            return _canCreateMerkleAirdrop(boardroom, data);
        }
        if (selector == DistributionFactoryVNext.createDutchAuction.selector) {
    ...
    function _canCreateMigratingBondingCurve(address boardroom, bytes calldata data) internal view returns (bool) {
        ...
        MigratingBondingCurveVNext.CreateParams memory params =
            abi.decode(data[4:], (MigratingBondingCurveVNext.CreateParams));
        if (params.shareToken != IDistributionBoardroom(boardroom).shareToken()) return false;
    ...
    function _canCreateMerkleAirdrop(address boardroom, bytes calldata data) internal view returns (bool) {
        ...
        MerkleAirdropVNext.CreateParams memory params = abi.decode(data[4:], (MerkleAirdropVNext.CreateParams));
        if (params.shareToken != IDistributionBoardroom(boardroom).shareToken()) return false;
    ...
    function _hasFeasibleCurveMigration(MigratingBondingCurveVNext.CreateParams memory params)
        internal
        pure
        returns (bool)
    {
    ...
    function _isCreateDistributionSelector(bytes4 selector) internal pure returns (bool) {
        return selector == DistributionFactoryVNext.createFixedPriceSale.selector
            || selector == DistributionFactoryVNext.createMigratingBondingCurve.selector
            || selector == DistributionFactoryVNext.createMerkleAirdrop.selector
            || selector == DistributionFactoryVNext.createDutchAuction.selector;
    }
    ...
    function _isMigratingBondingCurveLifecycleSelector(bytes4 selector) internal pure returns (bool) {
        return selector == MigratingBondingCurveVNext.cancel.selector;
    }

    function _isMerkleAirdropLifecycleSelector(bytes4 selector) internal pure returns (bool) {
        return selector == MerkleAirdropVNext.close.selector || selector == MerkleAirdropVNext.cancel.selector;
    }
    ...
    function _requireBoardroomShareToken(address boardroom, address shareToken) internal view {
        try IDistributionVNextBoardroomFactory(boardroomFactory).isBoardroom(boardroom) returns (bool canonical) {
