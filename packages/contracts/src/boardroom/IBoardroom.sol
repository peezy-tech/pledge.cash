...
import {BoardroomFacetTypes} from "./diamond/BoardroomFacetTypes.sol";

/// @notice Aggregate ABI for canonical release-bound Boardrooms.
...
interface IBoardroom {
    ...
    function launch(bytes32 expectedFacetSetHash, BoardroomFacetTypes.LaunchConfig calldata config) external;
    ...
    function execute(bytes32 expectedFacetSetHash, BoardroomFacetTypes.Call calldata call_)
        external
        payable
        returns (bytes memory result);

    function executeBatch(bytes32 expectedFacetSetHash, BoardroomFacetTypes.Call[] calldata calls)
        external
        payable
    ...
    function executeWindDownCall(bytes32 expectedFacetSetHash, BoardroomFacetTypes.Call calldata call_)
        external
        returns (bytes memory result);
    ...
    function returnProtocolLiquidityClaims(bytes32 expectedFacetSetHash) external returns (uint256 claims);
