...
interface IBoardroomVNextPrimaryMarketGuard {
...
/// @notice Boardroom share token for hash-bound vNext callbacks.
/// @dev Accounting and checkpoint behavior intentionally mirrors BoardroomToken.
contract BoardroomTokenVNext is ERC20 {
    struct Checkpoint {
        uint48 fromBlock;
        uint208 value;
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        IBoardroomVNextPrimaryMarketGuard guarded = IBoardroomVNextPrimaryMarketGuard(boardroom);
