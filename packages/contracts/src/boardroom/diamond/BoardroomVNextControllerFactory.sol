...
import {BoardroomVNextController} from "./BoardroomVNextController.sol";

interface ICanonicalVNextBoardroomFactory {
    function isBoardroom(address boardroom) external view returns (bool);
}

interface IBoardroomVNextControllerDeploymentAuthorizer {
...
/// @notice ABI-compatible controller factory used by the release-A legacy
/// Boardroom logic while deploying hash-bound vNext controllers.
contract BoardroomVNextControllerFactory {
    ...
    constructor(address boardroomFactory_) {
        ...
        controllerImplementation = address(new BoardroomVNextController());
    }
    ...
    function deployController(
        ...
        if (!ICanonicalVNextBoardroomFactory(boardroomFactory).isBoardroom(msg.sender)) {
        ...
        if (!IBoardroomVNextControllerDeploymentAuthorizer(msg.sender)
        ...
        BoardroomVNextController(controller).initialize(msg.sender, proposer, delay, gracePeriod, generation);
