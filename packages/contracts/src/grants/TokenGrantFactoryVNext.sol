...
contract TokenGrantFactoryVNext is Ownable, ERC721, IBoardroomObligationPolicy {
    ...
    function _isAuthorizedFactoryCall(address boardroom, bytes4 selector, uint256 value) internal view returns (bool) {
        if (selector == TokenGrantFactoryVNext.createGrant.selector) return value == creationFee;
        if (value != 0 || owner() != boardroom) return false;

        return selector == TokenGrantFactoryVNext.setCreationFee.selector
            || selector == TokenGrantFactoryVNext.setFeeRecipient.selector || selector == TRANSFER_OWNERSHIP_SELECTOR;
    }
    ...
    function _createsGrantObligation(address target, bytes calldata data, bytes calldata result)
        ...
        returns (bool)
    {
        return target == address(this) && _selector(data) == TokenGrantFactoryVNext.createGrant.selector
            && result.length == 32;
    }
    ...
    function _createGrant(GrantCreateInput memory input, uint256 fee) internal returns (address grant) {
        ...
        if (input.funder == input.issuer && isCanonicalBoardroom(input.issuer)) {
            ...
            if (input.token != shares) BoardroomVNextCallbackLib.reserveRedeemableAsset(input.issuer, input.token);
            if (input.paymentToken != address(0) && input.paymentToken != input.token) {
                BoardroomVNextCallbackLib.reserveRedeemableAsset(input.issuer, input.paymentToken);
            }
        }
