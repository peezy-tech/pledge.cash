...
import {BoardroomVNextCallbackLib, IBoardroomVNextCallbackTarget} from "../../src/policy/BoardroomVNextCallbackLib.sol";
...
contract VNextCallbackTargetMock is IBoardroomVNextCallbackTarget {
...
contract BoardroomVNextCallbackHarness {
    ...
    function precommitBondingCurve(address boardroom, address curve, address quoteAsset, uint256 fundingAmount)
        external
    {
        BoardroomVNextCallbackLib.precommitBondingCurve(boardroom, curve, quoteAsset, fundingAmount);
    }

    function recordGrantFromDistribution(address boardroom, address grant) external {
        BoardroomVNextCallbackLib.recordGrantFromDistribution(boardroom, grant);
    }

    function recordLockedLiquidityFromDistribution(address boardroom, address locker, address pool) external {
        BoardroomVNextCallbackLib.recordLockedLiquidityFromDistribution(boardroom, locker, pool);
    }

    function settleBondingCurve(address boardroom) external {
        BoardroomVNextCallbackLib.settleBondingCurve(boardroom);
    }
    ...
    function precommitProtocolLiquidity(
        ...
        BoardroomVNextCallbackLib.precommitProtocolLiquidity(
    ...
    function activateProtocolLiquidity(
        ...
        BoardroomVNextCallbackLib.activateProtocolLiquidity(boardroom, locker, pool, quoteAsset, curve, pairKey, salt);
    }

    function releaseProtocolLiquidityReservation(address boardroom, address curve, bytes32 pairKey, bytes32 salt)
        external
    {
        BoardroomVNextCallbackLib.releaseProtocolLiquidityReservation(boardroom, curve, pairKey, salt);
    }

    function closeProtocolLiquidityFromFactory(address boardroom, address locker) external {
        BoardroomVNextCallbackLib.closeProtocolLiquidityFromFactory(boardroom, locker);
    }
}

contract VNextCanonicalFactoryMock {
...
contract VNextPolicyRegistryMock {
...
contract VNextParityToken is ERC20 {
...
contract VNextAmmFactoryIdentityMock {
...
contract VNextAmmRouterIdentityMock {
...
contract BoardroomVNextModuleParityTest is Test {
    ...
    VNextCallbackTargetMock internal target;
    BoardroomVNextCallbackHarness internal harness;

    function setUp() public {
        target = new VNextCallbackTargetMock(RELEASE_A);
        harness = new BoardroomVNextCallbackHarness();
    }
    ...
    function testAssetAndDistributionCallbacksBindCurrentHashAndPreserveCaller() public {
        ...
        _assertCallback(VNextCallbackTargetMock.reserveRedeemableAsset.selector, RELEASE_A, 1);
        ...
        _assertCallback(VNextCallbackTargetMock.precommitBondingCurve.selector, RELEASE_B, 2);
        ...
        _assertCallback(VNextCallbackTargetMock.recordGrantFromDistribution.selector, RELEASE_B, 3);
        assertEq(target.addressArg0(), GRANT);

        harness.recordLockedLiquidityFromDistribution(address(target), LOCKER, POOL);
        _assertCallback(VNextCallbackTargetMock.recordLockedLiquidityFromDistribution.selector, RELEASE_B, 4);
        assertEq(target.addressArg0(), LOCKER);
        assertEq(target.addressArg1(), POOL);

        harness.settleBondingCurve(address(target));
        _assertCallback(VNextCallbackTargetMock.settleBondingCurve.selector, RELEASE_B, 5);
    }
    ...
    function testLiquidityCallbacksBindCurrentHashAndPreserveCaller() public {
        ...
        _assertCallback(VNextCallbackTargetMock.precommitProtocolLiquidity.selector, RELEASE_A, 1);
        assertEq(target.addressArg0(), LOCKER);
        assertEq(target.addressArg1(), QUOTE_ASSET);
        ...
        _assertCallback(VNextCallbackTargetMock.activateProtocolLiquidity.selector, RELEASE_B, 2);
        assertEq(target.addressArg0(), LOCKER);
        assertEq(target.addressArg1(), POOL);
        ...
        _assertCallback(VNextCallbackTargetMock.releaseProtocolLiquidityReservation.selector, RELEASE_B, 3);
        assertEq(target.addressArg0(), CURVE);
        assertEq(target.bytes32Arg0(), PAIR_KEY);
        assertEq(target.bytes32Arg1(), SALT);

        harness.closeProtocolLiquidityFromFactory(address(target), LOCKER);
        _assertCallback(VNextCallbackTargetMock.closeProtocolLiquidityFromFactory.selector, RELEASE_B, 4);
        assertEq(target.addressArg0(), LOCKER);
    }

    function testVNextFactoriesAcceptOneCoherentBoardroomIdentity() public {
        VNextCanonicalFactoryMock canonicalFactory = new VNextCanonicalFactoryMock();
        VNextAmmFactoryIdentityMock ammFactory = new VNextAmmFactoryIdentityMock(address(canonicalFactory));
        VNextAmmRouterIdentityMock ammRouter = new VNextAmmRouterIdentityMock(address(ammFactory));

        LockedLiquidityFactoryVNext liquidityFactory =
            new LockedLiquidityFactoryVNext(address(ammRouter), address(canonicalFactory));
        TokenGrantFactoryVNext grantFactory = new TokenGrantFactoryVNext(address(this), address(canonicalFactory));
        DistributionFactoryVNext distributionFactory =
            new DistributionFactoryVNext(address(liquidityFactory), address(grantFactory));
        BondMarketFactoryVNext bondFactory = new BondMarketFactoryVNext(address(ammFactory), address(canonicalFactory));
        BoardroomRewardsFactoryVNext rewardsFactory = new BoardroomRewardsFactoryVNext(address(canonicalFactory));
    ...
    function testVNextFactoriesRejectMixedBoardroomIdentities() public {
        VNextCanonicalFactoryMock canonicalFactory = new VNextCanonicalFactoryMock();
        VNextCanonicalFactoryMock otherFactory = new VNextCanonicalFactoryMock();
        VNextAmmFactoryIdentityMock ammFactory = new VNextAmmFactoryIdentityMock(address(canonicalFactory));
        VNextAmmRouterIdentityMock ammRouter = new VNextAmmRouterIdentityMock(address(ammFactory));

        vm.expectRevert(
            abi.encodeWithSelector(
                BondMarketFactoryVNext.IncoherentFactoryIdentity.selector,
                address(otherFactory),
                address(canonicalFactory)
            )
        );
        new BondMarketFactoryVNext(address(ammFactory), address(otherFactory));

        vm.expectRevert(
            abi.encodeWithSelector(
                LockedLiquidityFactoryVNext.IncoherentFactoryIdentity.selector,
                address(otherFactory),
                address(canonicalFactory)
            )
        );
        new LockedLiquidityFactoryVNext(address(ammRouter), address(otherFactory));

        LockedLiquidityFactoryVNext liquidityFactory =
            new LockedLiquidityFactoryVNext(address(ammRouter), address(canonicalFactory));
        TokenGrantFactoryVNext grantFactory = new TokenGrantFactoryVNext(address(this), address(otherFactory));
        vm.expectRevert(
            abi.encodeWithSelector(
                DistributionFactoryVNext.IncoherentFactoryIdentity.selector,
                address(otherFactory),
                address(canonicalFactory)
            )
        );
    ...
    function testVNextExternalBehaviorSelectorsMatchV5() public pure {
        _assertSelector(BondMarketFactory.createBondMarket.selector, BondMarketFactoryVNext.createBondMarket.selector);
        _assertSelector(BondMarketFactory.canCall.selector, BondMarketFactoryVNext.canCall.selector);
        _assertSelector(BondMarketFactory.obligationForCall.selector, BondMarketFactoryVNext.obligationForCall.selector);
        _assertSelector(
            BondMarketFactory.isLifecycleCallAllowed.selector, BondMarketFactoryVNext.isLifecycleCallAllowed.selector
        );
        _assertSelector(
            BondMarketFactory.predictBondMarketAddress.selector,
            BondMarketFactoryVNext.predictBondMarketAddress.selector
        );

        _assertSelector(
            BoardroomRewardsFactory.createRewards.selector, BoardroomRewardsFactoryVNext.createRewards.selector
        );
        _assertSelector(BoardroomRewardsFactory.fundReward.selector, BoardroomRewardsFactoryVNext.fundReward.selector);
        _assertSelector(BoardroomRewardsFactory.canCall.selector, BoardroomRewardsFactoryVNext.canCall.selector);
        _assertSelector(
            BoardroomRewardsFactory.obligationForCall.selector, BoardroomRewardsFactoryVNext.obligationForCall.selector
        );
        _assertSelector(
            BoardroomRewardsFactory.isLifecycleCallAllowed.selector,
            BoardroomRewardsFactoryVNext.isLifecycleCallAllowed.selector
        );
        _assertSelector(
            BoardroomRewardsFactory.predictRewardsAddress.selector,
            BoardroomRewardsFactoryVNext.predictRewardsAddress.selector
        );

        _assertSelector(TokenGrantFactory.setCreationFee.selector, TokenGrantFactoryVNext.setCreationFee.selector);
        _assertSelector(TokenGrantFactory.setFeeRecipient.selector, TokenGrantFactoryVNext.setFeeRecipient.selector);
        _assertSelector(TokenGrantFactory.createGrant.selector, TokenGrantFactoryVNext.createGrant.selector);
        _assertSelector(
            TokenGrantFactory.createGrantFromDistribution.selector,
            TokenGrantFactoryVNext.createGrantFromDistribution.selector
        );
        _assertSelector(
            TokenGrantFactory.predictGrantAddress.selector, TokenGrantFactoryVNext.predictGrantAddress.selector
        );
        _assertSelector(TokenGrantFactory.canCall.selector, TokenGrantFactoryVNext.canCall.selector);
        _assertSelector(TokenGrantFactory.obligationForCall.selector, TokenGrantFactoryVNext.obligationForCall.selector);
        _assertSelector(
            TokenGrantFactory.isLifecycleCallAllowed.selector, TokenGrantFactoryVNext.isLifecycleCallAllowed.selector
        );
        _assertSelector(
            TokenGrantFactory.isCanonicalBoardroom.selector, TokenGrantFactoryVNext.isCanonicalBoardroom.selector
        );
        _assertSelector(TokenGrantFactory.closeGrant.selector, TokenGrantFactoryVNext.closeGrant.selector);

        _assertSelector(
            DistributionFactory.createFixedPriceSale.selector, DistributionFactoryVNext.createFixedPriceSale.selector
        );
        _assertSelector(
            DistributionFactory.createDutchAuction.selector, DistributionFactoryVNext.createDutchAuction.selector
        );
        _assertSelector(
            DistributionFactory.createMigratingBondingCurve.selector,
            DistributionFactoryVNext.createMigratingBondingCurve.selector
        );
        _assertSelector(
            DistributionFactory.createMerkleAirdrop.selector, DistributionFactoryVNext.createMerkleAirdrop.selector
        );
        _assertSelector(DistributionFactory.canCall.selector, DistributionFactoryVNext.canCall.selector);
        _assertSelector(
            DistributionFactory.obligationForCall.selector, DistributionFactoryVNext.obligationForCall.selector
        );
        _assertSelector(
            DistributionFactory.isLifecycleCallAllowed.selector,
            DistributionFactoryVNext.isLifecycleCallAllowed.selector
        );
        _assertSelector(
            DistributionFactory.predictFixedPriceSaleAddress.selector,
            DistributionFactoryVNext.predictFixedPriceSaleAddress.selector
        );
        _assertSelector(
            DistributionFactory.predictDutchAuctionAddress.selector,
            DistributionFactoryVNext.predictDutchAuctionAddress.selector
        );
        _assertSelector(
            DistributionFactory.predictMigratingBondingCurveAddress.selector,
            DistributionFactoryVNext.predictMigratingBondingCurveAddress.selector
        );
        _assertSelector(
            DistributionFactory.predictMerkleAirdropAddress.selector,
            DistributionFactoryVNext.predictMerkleAirdropAddress.selector
        );

        _assertSelector(
            LockedLiquidityFactory.createLockedLiquidity.selector,
            LockedLiquidityFactoryVNext.createLockedLiquidity.selector
        );
        _assertSelector(
            LockedLiquidityFactory.createLockedLiquidityForBoardroom.selector,
            LockedLiquidityFactoryVNext.createLockedLiquidityForBoardroom.selector
        );
        _assertSelector(
            LockedLiquidityFactory.addLockedLiquidity.selector, LockedLiquidityFactoryVNext.addLockedLiquidity.selector
        );
        _assertSelector(
            LockedLiquidityFactory.removeLockedLiquidity.selector,
            LockedLiquidityFactoryVNext.removeLockedLiquidity.selector
        );
        _assertSelector(
            LockedLiquidityFactory.closeLockedLiquidity.selector,
            LockedLiquidityFactoryVNext.closeLockedLiquidity.selector
        );
        _assertSelector(
            LockedLiquidityFactory.finalizeWindDownClosure.selector,
            LockedLiquidityFactoryVNext.finalizeWindDownClosure.selector
        );
        _assertSelector(
            LockedLiquidityFactory.reserveMigration.selector, LockedLiquidityFactoryVNext.reserveMigration.selector
        );
        _assertSelector(
            LockedLiquidityFactory.releaseMigrationReservation.selector,
            LockedLiquidityFactoryVNext.releaseMigrationReservation.selector
        );
        _assertSelector(LockedLiquidityFactory.canCall.selector, LockedLiquidityFactoryVNext.canCall.selector);
        _assertSelector(
            LockedLiquidityFactory.obligationForCall.selector, LockedLiquidityFactoryVNext.obligationForCall.selector
        );
        _assertSelector(
            LockedLiquidityFactory.isLifecycleCallAllowed.selector,
            LockedLiquidityFactoryVNext.isLifecycleCallAllowed.selector
        );
        _assertSelector(
            LockedLiquidityFactory.predictLockedLiquidityAddress.selector,
            LockedLiquidityFactoryVNext.predictLockedLiquidityAddress.selector
        );
        ...
        _assertSelector(MigratingBondingCurve.initialize.selector, MigratingBondingCurveVNext.initialize.selector);
        _assertSelector(MigratingBondingCurve.buy.selector, MigratingBondingCurveVNext.buy.selector);
        _assertSelector(MigratingBondingCurve.sell.selector, MigratingBondingCurveVNext.sell.selector);
        _assertSelector(MigratingBondingCurve.migrate.selector, MigratingBondingCurveVNext.migrate.selector);
        _assertSelector(MigratingBondingCurve.cancel.selector, MigratingBondingCurveVNext.cancel.selector);
        _assertSelector(MigratingBondingCurve.expire.selector, MigratingBondingCurveVNext.expire.selector);
        _assertSelector(
            MigratingBondingCurve.fallbackToUnwind.selector, MigratingBondingCurveVNext.fallbackToUnwind.selector
        );
        _assertSelector(
            MigratingBondingCurve.finalizeUnwind.selector, MigratingBondingCurveVNext.finalizeUnwind.selector
        );
        _assertSelector(
            MigratingBondingCurve.recoverQuarantinedQuote.selector,
            MigratingBondingCurveVNext.recoverQuarantinedQuote.selector
        );
        _assertSelector(
            MigratingBondingCurve.openQuoteForfeiture.selector, MigratingBondingCurveVNext.openQuoteForfeiture.selector
        );
        _assertSelector(
            MigratingBondingCurve.vetoQuoteForfeiture.selector, MigratingBondingCurveVNext.vetoQuoteForfeiture.selector
        );
        _assertSelector(
            MigratingBondingCurve.finalizeQuoteForfeiture.selector,
            MigratingBondingCurveVNext.finalizeQuoteForfeiture.selector
        );
        _assertSelector(
            MigratingBondingCurve.recoverForfeitedQuote.selector,
            MigratingBondingCurveVNext.recoverForfeitedQuote.selector
        );
    }
...
contract BoardroomVNextRealModuleParityTest is Test {
    ...
    VNextCallbackTargetMock internal boardroom;
    VNextCanonicalFactoryMock internal canonicalFactory;
    VNextPolicyRegistryMock internal policyRegistry;
    VNextParityToken internal shares;
    VNextParityToken internal quote;
    WETH internal wrappedNative;
    AmmFactory internal ammFactory;
    AmmRouter internal ammRouter;
    LockedLiquidityFactoryVNext internal liquidityFactory;
    TokenGrantFactoryVNext internal grantFactory;
    DistributionFactoryVNext internal distributionFactory;
    BondMarketFactoryVNext internal bondFactory;
    BoardroomRewardsFactoryVNext internal rewardsFactory;

    function setUp() public {
        boardroom = new VNextCallbackTargetMock(RELEASE_HASH);
        canonicalFactory = new VNextCanonicalFactoryMock();
        policyRegistry = new VNextPolicyRegistryMock();
        shares = new VNextParityToken("Boardroom Share", "SHARE", 18, address(boardroom));
        quote = new VNextParityToken("Quote Asset", "QUOTE", 18, address(0));
        ...
        liquidityFactory = new LockedLiquidityFactoryVNext(address(ammRouter), address(canonicalFactory));
        ammFactory.setLiquidityRouter(address(ammRouter));
        ammFactory.setReservationManager(address(liquidityFactory));

        grantFactory = new TokenGrantFactoryVNext(address(this), address(canonicalFactory));
        distributionFactory = new DistributionFactoryVNext(address(liquidityFactory), address(grantFactory));
        bondFactory = new BondMarketFactoryVNext(address(ammFactory), address(canonicalFactory));
        rewardsFactory = new BoardroomRewardsFactoryVNext(address(canonicalFactory));
    ...
    function testRealFixedPriceAndDutchAuctionCreateTradeAndClose() public {
        ...
        assertEq(boardroom.lastSelector(), VNextCallbackTargetMock.reserveRedeemableAsset.selector);
    ...
    function testDistributionFactoryRejectsBoardroomShapedNonCanonicalCaller() public {
        ...
        vm.prank(address(boardroom));
        vm.expectRevert(abi.encodeWithSelector(DistributionFactoryVNext.InvalidBoardroom.selector, address(boardroom)));
    ...
    function testRealLockedLiquidityCreatesAddsRemovesAndCloses() public {
        ...
        LockedLiquidityFactoryVNext.CreateParams memory createParams = LockedLiquidityFactoryVNext.CreateParams({
        ...
        assertEq(boardroom.lastSelector(), VNextCallbackTargetMock.activateProtocolLiquidity.selector);
        ...
        LockedLiquidityFactoryVNext.AddParams memory addParams = LockedLiquidityFactoryVNext.AddParams({
            tokenA: address(shares),
            tokenB: address(quote),
        ...
        liquidityFactory.removeLockedLiquidity(
            LockedLiquidityFactoryVNext.RemoveParams({
        ...
        (,,, LockedLiquidityFactoryVNext.PositionStatus status) =
            liquidityFactory.positionOfBoardroom(address(boardroom));
        assertEq(uint256(status), uint256(LockedLiquidityFactoryVNext.PositionStatus.Closed));
        assertEq(boardroom.lastSelector(), VNextCallbackTargetMock.closeProtocolLiquidityFromFactory.selector);
        assertEq(boardroom.lastCaller(), address(liquidityFactory));
    }
    ...
    function testRealMigratingCurveCreatesTradesCancelsAndSettles() public {
        ...
        vm.prank(address(boardroom));
        MigratingBondingCurveVNext curve =
            MigratingBondingCurveVNext(distributionFactory.createMigratingBondingCurve(params));
        ...
        assertEq(boardroom.lastSelector(), VNextCallbackTargetMock.precommitProtocolLiquidity.selector);
        assertEq(boardroom.lastCaller(), address(liquidityFactory));
        ...
        vm.prank(address(boardroom));
        curve.cancel();
        assertEq(uint256(curve.curveStatus()), uint256(MigratingBondingCurveVNext.CurvePhase.Unwinding));

        vm.warp(curve.phaseEndsAt() + 1);
        curve.finalizeUnwind();
        assertTrue(curve.isClosed());
        assertFalse(curve.migrationReservationHeld());
        assertEq(boardroom.lastSelector(), VNextCallbackTargetMock.settleBondingCurve.selector);
    ...
    function _grantClaimLeaf(
        ...
        address account,
        uint256 amount,
        MerkleAirdropVNext.GrantClaimParams memory params
