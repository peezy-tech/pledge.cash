...
contract ControllerReentrantTarget {
    function reenter(
        ...
        controller.executeBoardroomOperation(calls, salt, boardroomEpoch, configurationEpoch, authority);
    }
}
...
contract BoardroomControllerTest is Test {
    ...
    function testControllerDoesNotExistUntilAtomicLaunchAndFactoryRequiresLaunchContext() public {
        (Boardroom boardroom,) = _prepareBoardroom("atomic-launch", 100 ether, protection);
    ...
    function testLaunchChecksNamedProtectionAndReciprocalConfiguration() public {
        (Boardroom boardroom, BoardroomRewards rewards) = _createBoardroom("launch-validation");
        address weakProtection = address(0x1111);

        vm.startPrank(owner);
        boardroom.mint(protection, 91 ether);
        boardroom.mint(weakProtection, 9 ether);
        vm.stopPrank();
        vm.prank(protection);
        ...
        Boardroom.LaunchConfig memory config =
        ...
        vm.prank(owner);
        vm.expectRevert(Boardroom.InvalidLaunchConfiguration.selector);
        boardroom.launch(config);

        config.expectedRewardPool = address(rewards);
        config.predictedController = stranger;
        vm.prank(owner);
        vm.expectRevert(abi.encodeWithSelector(Boardroom.InvalidController.selector, stranger));
        boardroom.launch(config);
    ...
    function testLaunchRejectsEverySecurityRelevantMismatchAndLeavesNoPartialController() public {
        (Boardroom boardroom,) = _prepareBoardroom("launch-mismatch-matrix", 100 ether, protection);
        Boardroom.LaunchConfig memory config = _launchConfig(boardroom, proposer, protection, 1 days, 3 days, 2 days);
        ...
        vm.prank(owner);
        vm.expectRevert(abi.encodeWithSelector(Boardroom.InvalidController.selector, config.predictedController));
        boardroom.launch(config);
        config.predictedController = predicted;
    ...
    function testProtectionStakerRequiresCurrentPastAndUnencumberedPower() public {
        ...
        {
            (Boardroom boardroom, BoardroomRewards rewards) = _createBoardroom("protection-current-mismatch");
            vm.prank(owner);
            boardroom.mint(protection, 10 ether);
            vm.prank(protection);
            rewards.stake(10 ether);
            vm.roll(block.number + 1);
            vm.prank(owner);
            boardroom.mint(otherHolder, 100 ether);
        ...
        {
            (Boardroom boardroom, BoardroomRewards rewards) = _createBoardroom("protection-past-mismatch");
            vm.startPrank(owner);
            boardroom.mint(protection, 10 ether);
            boardroom.mint(otherHolder, 90 ether);
            vm.stopPrank();
            vm.prank(protection);
        ...
        {
            (Boardroom boardroom, BoardroomRewards rewards) = _createBoardroom("protection-encumbered");
            ...
            vm.startPrank(owner);
            boardroom.mint(address(obligation), 100 ether);
            boardroom.mint(otherHolder, 100 ether);
            vm.stopPrank();
    ...
    function testFactoryBindingRejectsReleaseMismatchPreemptionAndAddressOccupation() public {
        (Boardroom victim,) = _prepareBoardroom("factory-hardening-victim", 100 ether, protection);
        (Boardroom otherBoardroom,) = _createBoardroom("factory-hardening-other");
        ...
        Boardroom.LaunchConfig memory config = _launchConfig(victim, proposer, protection, 1 days, 1 days, 1 days);
        vm.etch(predicted, hex"60006000fd");
        vm.prank(owner);
        vm.expectRevert(abi.encodeWithSelector(Boardroom.ControllerAlreadyDeployed.selector, predicted));
        victim.launch(config);
    ...
    function testOnlyProposerSchedulesAndAnyoneExecutesWithProposerAsPolicyAuthority() public {
        (Boardroom boardroom, BoardroomController controller,) = _launchedBoardroom("schedule-execute", proposer);
        ...
        controller.scheduleBoardroomOperation(calls, salt, 1, 1);

        vm.prank(proposer);
        (bytes32 operationId, uint256 eta) = controller.scheduleBoardroomOperation(calls, salt, 1, 1);

        vm.prank(executor);
        vm.expectRevert(
            abi.encodeWithSelector(BoardroomController.OperationNotReady.selector, operationId, eta, block.timestamp)
        );
        controller.executeBoardroomOperation(calls, salt, 1, 1, proposer);
        ...
        controller.executeBoardroomOperation(calls, salt, 1, 1, executor);
    ...
    function testOnlyPolicyCheckedGatewayCanMutateAndControllerExecutionRegistersObligations() public {
        (Boardroom boardroom, BoardroomController controller,) = _launchedBoardroom("gateway-and-obligation", proposer);
...

        {
            ...
            vm.prank(proposer);
            vm.expectRevert(Ownable.Unauthorized.selector);
            boardroom.executeGovernance(1, proposer, directCalls);
        }

        {
            ...
            (bytes32 rejectedId, uint256 rejectedEta) =
                controller.scheduleBoardroomOperation(rejectedCalls, rejectedSalt, 1, 1);
            vm.warp(rejectedEta);
            vm.expectRevert();
            controller.executeBoardroomOperation(rejectedCalls, rejectedSalt, 1, 1, proposer);
        ...
        vm.prank(proposer);
        (, uint256 obligationEta) = controller.scheduleBoardroomOperation(obligationCalls, obligationSalt, 1, 1);
        vm.warp(obligationEta);
        vm.prank(executor);
        controller.executeBoardroomOperation(obligationCalls, obligationSalt, 1, 1, proposer);
    ...
    function testSchedulingBoundsExpiryReplayAndPayloadBinding() public {
        ...
        vm.prank(proposer);
        vm.expectRevert(BoardroomController.InvalidConfiguration.selector);
        controller.scheduleBoardroomOperation(empty, bytes32(0), 1, 1);
        ...
        controller.scheduleBoardroomOperation(tooMany, bytes32(0), 1, 1);
        ...
        assertNotEq(operationId, controller.hashBoardroomOperation(changedCalls, salt, 1, 1, proposer));
        assertNotEq(operationId, controller.hashBoardroomOperation(calls, keccak256("other-salt"), 1, 1, proposer));
        assertNotEq(operationId, controller.hashBoardroomOperation(calls, salt, 2, 1, proposer));
        assertNotEq(operationId, controller.hashBoardroomOperation(calls, salt, 1, 2, proposer));
        assertNotEq(operationId, controller.hashBoardroomOperation(calls, salt, 1, 1, executor));
        ...
        vm.prank(proposer);
        (bytes32 replayId, uint256 replayEta) = replayController.scheduleBoardroomOperation(calls, salt, 1, 1);
        vm.warp(replayEta);
        replayController.executeBoardroomOperation(calls, salt, 1, 1, proposer);
    ...
    function testHolderVetoUsesCurrentAndPastStakeAndOnlyBoardroomCancels() public {
        (Boardroom boardroom, BoardroomController controller,) = _launchedBoardroom("holder-veto", proposer);
    ...
    function testHolderVetoCancelsSelfConfigurationImmediatelyBeforeAndAtReadiness() public {
        (Boardroom boardroom, BoardroomController controller,) =
        ...
        vm.prank(proposer);
        (bytes32 beforeId, uint256 beforeEta) = controller.scheduleControllerOperation(updateData, beforeSalt, 1, 1);
        vm.warp(beforeEta - 1);
        vm.prank(protection);
        boardroom.veto(beforeId);
        vm.warp(beforeEta);
        vm.expectRevert(abi.encodeWithSelector(BoardroomController.OperationNotPending.selector, beforeId));
        controller.executeControllerOperation(updateData, beforeSalt, 1, 1, proposer);

        bytes32 readySalt = keccak256("self-veto-at-ready");
        vm.prank(proposer);
        (bytes32 readyId, uint256 readyEta) = controller.scheduleControllerOperation(updateData, readySalt, 1, 1);
        vm.warp(readyEta);
        vm.prank(protection);
        boardroom.veto(readyId);
        vm.expectRevert(abi.encodeWithSelector(BoardroomController.OperationNotPending.selector, readyId));
        controller.executeControllerOperation(updateData, readySalt, 1, 1, proposer);
    ...
    function testConfigurationChangesAreDelayedSelfGovernanceAndInvalidateOldOperations() public {
        ...
        vm.prank(proposer);
        (bytes32 staleId,) = controller.scheduleBoardroomOperation(staleCalls, staleSalt, 1, 1);
        vm.prank(proposer);
        (, uint256 eta) = controller.scheduleControllerOperation(updateData, keccak256("update"), 1, 1);

        vm.warp(eta);
        vm.prank(executor);
        controller.executeControllerOperation(updateData, keccak256("update"), 1, 1, proposer);
        ...
        controller.executeBoardroomOperation(staleCalls, staleSalt, 1, 1, proposer);
        ...
        vm.prank(proposer);
        vm.expectRevert(BoardroomController.OnlyProposer.selector);
        controller.scheduleBoardroomOperation(staleCalls, keccak256("old-proposer"), 1, 2);
        vm.prank(stranger);
        controller.scheduleBoardroomOperation(staleCalls, keccak256("new-proposer"), 1, 2);
    }

    function testWindDownAdvancesEpochAndInvalidatesAllPendingOperationsInConstantTime() public {
        (Boardroom boardroom, BoardroomController controller,) = _launchedBoardroom("wind-down-epoch", proposer);
        BoardroomCall[] memory calls = _targetCalls(address(allowAllPolicy), 31);
        bytes32 salt = keccak256("wind-down-stale");
        vm.prank(proposer);
        (bytes32 operationId,) = controller.scheduleBoardroomOperation(calls, salt, 1, 1);
    ...
    function testWindDownInvalidatesManyOperationsInConstantTimeAndPreservesIndependentDelay() public {
        (Boardroom boardroom, BoardroomController controller, BoardroomRewards rewards) =
        ...
        vm.prank(proposer);
        (, uint256 updateEta) = controller.scheduleControllerOperation(updateData, updateSalt, 1, 1);
        vm.warp(updateEta);
        controller.executeControllerOperation(updateData, updateSalt, 1, 1, proposer);
        ...
        controller.executeBoardroomOperation(
            calls, keccak256(abi.encode("queued-before-wind-down", uint256(95))), 1, 1, proposer
        );
        ...
        rewards.terminalize();
        assertTrue(boardroom.pruneObligation(address(rewards)));
    ...
    function testReplacementDeploysNextGenerationOnlyInsideAtomicSelfCall() public {
        (Boardroom boardroom, BoardroomController controller,) = _launchedBoardroom("replacement", proposer);
        ...
        vm.prank(proposer);
        controller.scheduleBoardroomOperation(staleCalls, keccak256("old-generation"), 1, 1);
        ...
        replacementCalls[0] = BoardroomCall({
            ...
            data: abi.encodeCall(
                ...
                (address(controller), predictedNext, stranger, uint64(2 days), uint64(4 days), uint64(2))
            )
        });
        vm.prank(proposer);
        (, uint256 eta) = controller.scheduleBoardroomOperation(replacementCalls, keccak256("replace"), 1, 1);
        assertEq(predictedNext.code.length, 0);

        vm.warp(eta);
        vm.prank(executor);
        controller.executeBoardroomOperation(replacementCalls, keccak256("replace"), 1, 1, proposer);
        ...
        controller.executeBoardroomOperation(staleCalls, keccak256("old-generation"), 1, 1, proposer);

        vm.prank(stranger);
        next.scheduleBoardroomOperation(staleCalls, keccak256("new-generation"), 2, 1);
    }

    function testReplacementCannotBeCalledDirectlyOrHiddenInsidePolicyWrappedBatch() public {
        (Boardroom boardroom, BoardroomController controller,) = _launchedBoardroom("replacement-atomicity", proposer);
        address predictedNext = controllerFactory.predictControllerAddress(address(boardroom), 2);
        bytes memory replacementData = abi.encodeCall(
            Boardroom.replaceController,
            (address(controller), predictedNext, stranger, uint64(2 days), uint64(2 days), uint64(2))
        );

        vm.prank(proposer);
        vm.expectRevert(Ownable.Unauthorized.selector);
        boardroom.replaceController(address(controller), predictedNext, stranger, 2 days, 2 days, 2);
        vm.prank(address(controller));
        vm.expectRevert(Ownable.Unauthorized.selector);
        boardroom.replaceController(address(controller), predictedNext, stranger, 2 days, 2 days, 2);
        ...
        (bytes32 operationId, uint256 eta) = controller.scheduleBoardroomOperation(calls, salt, 1, 1);
        vm.warp(eta);
        vm.expectRevert(Boardroom.InvalidControllerReplacement.selector);
        controller.executeBoardroomOperation(calls, salt, 1, 1, proposer);
    ...
    function testOwnershipTransferAndRenunciationRemainDisabledAfterLaunch() public {
        (Boardroom boardroom, BoardroomController controller,) = _launchedBoardroom("ownership-lock", proposer);

        vm.prank(address(controller));
        vm.expectRevert(Boardroom.OwnershipLockedAfterLaunch.selector);
        ...
        vm.prank(stranger);
        vm.expectRevert(Boardroom.OwnershipLockedAfterLaunch.selector);
        boardroom.cancelOwnershipHandover();
        vm.prank(address(controller));
        vm.expectRevert(Boardroom.OwnershipLockedAfterLaunch.selector);
    ...
    function testControllerERC1271AcceptsEOAProposerAndRejectsWrongSigner() public {
        ...
        bytes memory signature = _sign(proposerKey, digest);

        assertEq(controller.isValidSignature(digest, signature), controller.ERC1271_MAGIC_VALUE());
        assertEq(controller.isValidSignature(digest, _sign(0xBAD123, digest)), bytes4(0xffffffff));

        BoardroomCall[] memory calls = _targetCalls(address(allowAllPolicy), 51);
        bytes32 operationId = controller.hashBoardroomOperation(calls, keccak256("signature-only"), 1, 1, eoaProposer);
    ...
    function testControllerERC1271RecursesThroughContractProposer() public {
        ...
        assertEq(controller.isValidSignature(digest, _sign(signerKey, digest)), controller.ERC1271_MAGIC_VALUE());
        assertEq(controller.isValidSignature(digest, _sign(0xDEAD123, digest)), bytes4(0xffffffff));

        BoardroomCall[] memory calls = _targetCalls(address(allowAllPolicy), 61);
        vm.prank(address(safe));
        controller.scheduleBoardroomOperation(calls, keccak256("safe-schedules"), 1, 1);
    }
    ...
    function testControllerExecutionIsReentrancySafeAndRollsBackConsumption() public {
        ...
        vm.prank(proposer);
        (bytes32 innerId, uint256 eta) = controller.scheduleBoardroomOperation(innerCalls, innerSalt, 1, 1);
        ...
        outerCalls[0] = BoardroomCall({
            ...
            value: 0,
            data: abi.encodeCall(
                reentrant.reenter, (controller, innerCalls, innerSalt, uint256(1), uint256(1), proposer)
            )
        });
        bytes32 outerSalt = keccak256("outer");
        vm.prank(proposer);
        (bytes32 outerId,) = controller.scheduleBoardroomOperation(outerCalls, outerSalt, 1, 1);

        vm.warp(eta);
        vm.expectRevert();
        controller.executeBoardroomOperation(outerCalls, outerSalt, 1, 1, proposer);
        ...
        controller.executeBoardroomOperation(innerCalls, innerSalt, 1, 1, proposer);
        assertEq(target.value(), 71);
    }

    function _launchedBoardroom(string memory label, address proposer_)
        internal
        returns (Boardroom boardroom, BoardroomController controller, BoardroomRewards rewards)
    {
    ...
    function _prepareBoardroom(string memory label, uint256 amount, address staker)
        internal
        returns (Boardroom boardroom, BoardroomRewards rewards)
    {
        (boardroom, rewards) = _createBoardroom(label);
        vm.prank(owner);
        boardroom.mint(staker, amount);
        vm.prank(staker);
        rewards.stake(amount);
        vm.roll(block.number + 1);
    }

    function _createBoardroom(string memory label) internal returns (Boardroom boardroom, BoardroomRewards rewards) {
        bytes32 salt = keccak256(abi.encode(label, ++serial));
        boardroom = Boardroom(payable(boardroomFactory.createBoardroom(owner, "Controller Boardroom", "CTRL", salt)));
        vm.prank(owner);
    ...
    function _launch(
        ...
        Boardroom.LaunchConfig memory config = _launchConfig(
    ...
    function _launchConfig(
        ...
        uint64 windDownDelay,
        uint64 gracePeriod
    ) internal view returns (Boardroom.LaunchConfig memory config) {
        config = Boardroom.LaunchConfig({
            proposer: proposer_,
    ...
    function _assertRecordedDistribution(Boardroom boardroom, ControllerObligationPolicy obligationPolicy)
        internal
        view
    {
        ControllerTestObligation obligation = obligationPolicy.obligation();
