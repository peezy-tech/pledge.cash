...
contract BoardroomInvariantERC20 is ERC20 {
    function name() public pure override returns (string memory) {
        return "Boardroom Redeemable";
    }
...
contract BoardroomWindDownHandler is Test {
    ...
    BoardroomFactory public immutable factory;
    IBoardroom public immutable boardroom;
    BoardroomToken public immutable shareToken;
    BoardroomInvariantERC20 public immutable redeemableAsset;
    BoardroomToggleAsset public immutable hostileAsset;
    ...
    constructor() {
        ...
        BoardroomFactory factory_ = new BoardroomFactory(
            address(registry_),
        ...
        address controllerFactory = factory_.controllerFactory();
        ...
        ProtocolFacetTypes.FacetSetManifest memory releaseB = BoardroomRelease.releaseB(facets, releaseAHash_);
        bytes32 releaseBHash_ = registry_.publishFacetSet(releaseB);

        IBoardroom boardroom_ = IBoardroom(
            factory_.createBoardroom(
                releaseAHash_, address(this), "Boardroom Invariant", "BINV", keccak256("boardroom-wind-down-invariant")
            )
        );
        BoardroomToken shareToken_ = BoardroomToken(boardroom_.shareToken());
        BoardroomInvariantERC20 redeemableAsset_ = new BoardroomInvariantERC20();
        BoardroomToggleAsset hostileAsset_ = new BoardroomToggleAsset();
    ...
    function startWindDown() external {
        if (boardroom.status() != BoardroomFacetTypes.BoardroomStatus.Active) return;
        try boardroom.startWindDown(registry.activeFacetSetHash()) {} catch {}
    }

    function burnTreasuryShares() external {
        if (boardroom.status() != BoardroomFacetTypes.BoardroomStatus.WindingDown) return;
    ...
    function snapshotAssets(uint256 rawMaximum) external {
        if (boardroom.status() != BoardroomFacetTypes.BoardroomStatus.Snapshotting) return;
    ...
    function redeem(uint256 actorSeed, uint256 sharesSeed) external {
        if (boardroom.status() != BoardroomFacetTypes.BoardroomStatus.RedemptionsOpen || boardroom.migrationRequired()) return;
...
contract BoardroomWindDownInvariantTest is StdInvariant, Test {
    BoardroomWindDownHandler internal handler;

    function setUp() public {
        handler = new BoardroomWindDownHandler();
    ...
    function testHostileAssetFailureDoesNotBlockHealthyPayoutAndCanRetryAcrossReleaseMigration() public {
        ...
        assertEq(uint8(handler.boardroom().status()), uint8(BoardroomFacetTypes.BoardroomStatus.Active));
    ...
    function invariantTreasurySharesAreBurnedBeforeRedemptionsOpen() public view {
        if (handler.boardroom().status() == BoardroomFacetTypes.BoardroomStatus.RedemptionsOpen) {
