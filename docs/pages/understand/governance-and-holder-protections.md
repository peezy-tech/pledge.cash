...
Boardroom governance separates proposing, waiting, reviewing, and executing.
It is part of canonical protocol v1, which is pending on both target testnets
and is not deployed or certified for mainnet.
...
Launch requires the named protection staker to meet the 10%
current-and-previous-block threshold. It deploys and verifies generation 1 and
transfers ownership atomically. It also requires the caller's explicit active
facet-set hash and a Boardroom whose storage migration is complete.
...
- Every operation binds the expected facet-set hash, full call batch or
  self-call, salt, Boardroom epoch, controller generation, configuration
...
The Boardroom is the controller's immutable canceller. A qualified holder
calls `Boardroom.veto(expectedFacetSetHash, operationId)`; there is no
privileged administrator or emergency-delay bypass.
...
Operation history is trustworthy only when deployment and registry identity,
facet-set hash, migration state, controller identity, epochs, and event scans
are complete. `Unknown` is not zero. Pending or mismatched releases fail
closed for governance writes.
